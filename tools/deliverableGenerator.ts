...
const FirebasePushPayloadSchema = z.object({
    notification: z.object({
        title: z.string().describe("Notification title (placeholder, user should replace)"),
        body: z.string().describe("Notification body (placeholder, user should replace)"),
    }),
    data: z.object({
        href: z.string().describe("The full gesund:// deeplink"),
        ...
        linkLabel: z.string().optional().describe("Optional label for a button in the push notification"),
    }),
...
const DeliverableGeneratorInputSchema = z.object({
    confirmedPath: z.string().describe("The validated canonical path (e.g., 'profile/orders', 'details/product/:id'). Must not contain unsubstituted placeholders like /:id."),
    parameters: z.array(z.object({ key: z.string(), value: z.string() })).optional().describe("List of parameter key/value pairs (e.g., [{ key: 'id', value: '12345' }])."),
    requestedDeliverables: z.array(z.enum(["deeplink", "adjust_steps", "firebase_push", "qr_code"])).min(1).describe("List of asset types requested by the user."),
});

// Define output schema
const DeliverableGeneratorOutputSchema = z.object({
    fullDeeplink: z.string().nullable().describe("The complete gesund:// deeplink, or null if not requested/applicable."),
    adjustSteps: z.string().nullable().describe("Markdown text guiding the user through Adjust Campaign Lab UI steps, or null if not requested."),
    ...
    qrCodeInfo: z.string().nullable().describe("Instructions or a URL for generating the QR code, or null if not requested."),
    testingChecklist: z.string().describe("Standard testing checklist text."),
    notes: z.string().nullable().describe("Any additional notes or warnings (e.g., Dynamic Links deprecation)."),
    error: z.string().nullable().describe("Error message if generation failed (e.g., path still contains placeholders)."),
});

// Revert to factory function
export function createDeliverableGeneratorTool(aiInstance: Genkit): ToolAction<typeof DeliverableGeneratorInputSchema, typeof DeliverableGeneratorOutputSchema> {
    return aiInstance.defineTool(
        ...
        async (input: z.infer<typeof DeliverableGeneratorInputSchema>): Promise<z.infer<typeof DeliverableGeneratorOutputSchema>> => {
            let fullDeeplink: string | null = null;
            let adjustSteps: string | null = null;
            let firebasePushPayload: z.infer<typeof FirebasePushPayloadSchema> | null = null;
            let qrCodeInfo: string | null = null;
            const notes: string[] = [];
